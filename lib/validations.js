...
function validationFailed(inst, attr, conf, options, cb) {
  ...
  if (skipValidation(inst, conf, 'if', options) ||
...
function skipValidation(inst, conf, kind, options) {
  var doValidate = true;
  if (typeof conf[kind] === 'function') {
    doValidate = conf[kind].call(inst, options);
...
function ValidationError(obj) {
  ...
  this.message = g.f(
    'The %s instance is not valid. Details: %s.',
      ...
      this.formatErrors(obj.errors, obj.toJSON()) || '(unknown)'
  );
...
ValidationError.prototype.formatPropertyError = formatPropertyError;
ValidationError.prototype.formatErrors = formatErrors;
ValidationError.prototype.truncatePropertyString = truncatePropertyString;
...
function formatErrors(errors, propertyValues) {
  ...
  return Object.getOwnPropertyNames(errors)
    ...
    .map(function(propertyName) {
      ...
      var propertyValue = _.get(propertyValues, propertyName);// propertyValues[propertyName];
      return messages.map(function(msg) {
        return formatPropertyError.call(that, propertyName, propertyValue, msg);
      }).join(DELIM);
    })
    .join(DELIM);
}
...
function formatPropertyError(propertyName, propertyValue, errorMessage) {
  ...
  if (valueType === 'string') {
    formattedValue = JSON.stringify(this.truncatePropertyString(propertyValue, propertyName));
    ...
    formattedValue = this.truncatePropertyString(formattedValue);
  } else {
    formattedValue = this.truncatePropertyString('' + propertyValue);
  }

  propertyName = _.chain(propertyName).split('.').last().value();
